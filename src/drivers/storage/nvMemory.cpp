...
bool nvMemory::saveConfig(TSettings* Settings)
{
    if (init())
    {
        ...
        json[JSON_SPIFFS_KEY_SCREENTIMEOUT] = Settings->ScreenTimeoutS;
        ...
        if (SPIFFS.exists(JSON_CONFIG_FILE))
        {
            ...
            if (configFile)
            {
                ...
                if (!error)
                {
                    ...
                    if (json.containsKey(JSON_SPIFFS_KEY_SCREENTIMEOUT)) {
                        Settings->ScreenTimeoutS = json[JSON_SPIFFS_KEY_SCREENTIMEOUT].as<int>();
                    } else {
                        Settings->ScreenTimeoutS = DEFAULT_SCREENTIMEOUT;
                    }
                    return true;
                }
                else
